...
export class JpgConverterService extends BaseConverterService {
    ...
    protected async convertContent(source: File): Promise<string> {
        ...
        const template = await this.templateService.loadTemplate(
            this.settingsService.jpgTemplatePath,
            DEFAULT_IMAGE_TEMPLATE
        );

        // Prepare template variables
        const variables = {
            title: source.name.replace(/\.jpg$/i, ''),
    ...
    override async convertFiles(): Promise<void> {
        if (!this.parser.validateApiKey()) {
            console.warn('[JpgConverter] No Google API key configured - JPG image parsing will be skipped');
            new Notice('⚠️ Google API key not configured. JPG files will not be indexed.\n\nPlease add your API key in plugin settings to enable image processing.', 8000);
            return;
        }
